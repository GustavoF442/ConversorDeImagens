...
export default function App() {
  ...
  const handleSelectFolder = useCallback(async () => {
    ...
    showNotification('success', `Found ${result.count} images (${formatSize(result.totalSize)})`);
  }, [showNotification]);
  ...
  const handleSelectFiles = useCallback(async () => {
    ...
    showNotification('success', `Selected ${fileInfos.length} images`);
  }, [showNotification]);
  ...
  const handleSelectOutput = useCallback(async () => {
    ...
    if (folder) {
      setOutputDir(folder);
      showNotification('success', `Output: ${folder}`);
    }
  }, [showNotification]);

  // Load preview when file or settings change
  useEffect(() => {
    if (!selectedFile || !window.api) return;

    // Set original preview as file path
  ...
  const handleProcessBatch = useCallback(async () => {
    ...
    try {
      ...
      showNotification('success', `Processed ${result.success} images successfully`);
      if (result.errors.length > 0) {
        console.error('Batch errors:', result.errors);
      }
    } catch (err: any) {
      showNotification('error', `Batch processing failed: ${err.message}`);
    } finally {
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    if (imageFiles.length > 0) {
      ...
      showNotification('success', `Added ${imageFiles.length} images`);
    }
      ...
      <div className="app-layout">
        ...
        <div className="main-content">
          <div className="toolbar">
            ...
            >
              <ZapIcon /> Process All ({files.length})
            </button>
            <div className="toolbar-info">
              {outputDir ? `Output: ${outputDir.split(/[\\/]/).pop()}` : 'No output folder selected'}
            </div>
          </div>
